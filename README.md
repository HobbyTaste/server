...
Чтобы запустить локально проект, необходимо, имея локально установленными репозиториями с frontend и с backend, из корневой директории данного проекта:
1. Сначала установить все зависимости проекта командой:
```shell script
yarn 
```
2. Далее запускаем сборку. Для корректной сборки необходимо иметь переменную окружения `FRONT_PATH` с путем до репозитория с frontend. Чтобы собрать и клиентскую, и серверную часть с продовыми настройками необходимо запустить команду:
```shell script
yarn build
``` 
или можно задать переменную окружения на месте:
```shell script
FRONT_PATH=path/to/front yarn build
```
Замечание: Если два репозитория лежат рядом, то переменную можно задать следующим образом: `FRONT_PATH=../web-application`
