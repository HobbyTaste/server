...
Локальный запуск backend-сервера вместе с nginx балансером:
...
yarn start:watch
```
В консоли пишется порт, по которому поднимается сервер, и порт, по которому запускается балансер.
